            ...
            <div class="grid grid-cols-1 mx-auto text-white font-bold w-full">
                ...
                <div class="grid grid-rows-1 w-5/6 lg:w-1/2 mx-auto gap-2">
                    ...
                    @foreach ($producto->MostrarColores() as $color)
                        <div class="flex flex-cols-2 text-sm contenedor-colores p-2">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                @foreach ($producto->MostrarTalles($color->id) as $talle)
                                    <div class="w-full">
                                        <label class="w-full grid grid-cols-1 bg-white mx-auto rounded-xl text-black gap-2 hover:cursor-pointer hover:bg-black hover:text-white border-solid border-2 border-sky-500">
                                            <p class="mx-auto p-2">{{ $talle->talle }}</p>
                                            <input onclick="pintar_2(this)" class="hidden" type="checkbox" id="{{ $talle->id }}" value="{{ $talle->talle }}">
                                        </label>
                                        <div class="hidden mt-2 ">
                                            <button onclick="restar(this)" class="bg-black text-white rounded px-2 hover:pointer-cursor">-</button>
                                            <input type="number" name="cantidad" class="text-black text-center p-0 rounded border-sky-500 w-9" readonly value="0">
                                            <button onclick="sumar(this)" class="bg-black text-white rounded px-2 hover:pointer-cursor">+</button>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
...
<script>
    ...
    function pintar_2(e){
        const contenedor = e.parentNode.parentNode.children[1];
        const input = contenedor.children[1];
    ...
    function MensajesExitosErrores(mensaje, color, opacidad, limpiar = false){
    ...
    const CheckAgregar = (id) => {
        ...
        var cont_colors = 0;
        var cont_talles = 0;
        MensajesExitosErrores("", "", "", true);

        var contenedores = document.querySelectorAll('.contenedor-colores');
        let cortar = true;
        let checked_colores = 0;
        contenedores.forEach((contenedor) => { 
            var input_color = contenedor.children[0].children[0].children[1];
            var arr_content = contenedor.children[1].children;
            var contenedor_talles = contenedor.children[1];
            contenedor.classList = "flex flex-cols-2 text-sm contenedor-colores p-2";
            if(input_color.checked){
                checked_colores++;
                var color = { nombre_color: input_color.value, id_color: input_color.id, talles: [] };
                let checked = 0;
                contenedor_talles.classList = "grid grid-cols-2 gap-2";

                Array.from(arr_content).forEach((element) => {
                    var input_talle = element.children[0].children[1]; // input talle
                    var input_cantidad = element.children[1].children[1]; // input cantidad
                    if(input_talle.checked){
                        color.talles.push({
                            nombre_talle: input_talle.value,
                            id_talle: input_talle.id,
                ...
                if(checked)
                    colores_arr.push(color);

                if(!checked){
                    contenedor_talles.classList = "grid grid-cols-2 gap-2 border border-2 border-red-500 rounded";
                    MensajesExitosErrores("Debes elegir un talle", "red", "600");
                    cortar = false;
                }
            }else{
                let checked = 0;
                Array.from(arr_content).forEach((element) => {
                    var input_talle = element.children[0].children[1]; // input talle
                    element.parentNode.classList = "grid grid-cols-2 gap-2";
                    if(input_talle.checked)
                        checked++

                    if(checked){
                        element.parentNode.classList = "grid grid-cols-2 gap-2 border border-2 border-red-500 rounded";
                        contenedor.classList = "flex flex-cols-2 text-sm contenedor-colores p-2 border border-2 border-red-500 rounded";
                        MensajesExitosErrores("Debes elegir un talle", "red", "600");
                        cortar = false;
                    }
                });
        ...
        if(!cortar || !checked_colores){
            if(!checked_colores){
                MensajesExitosErrores("Debes elegir un color", "red", "600");
            }
            return
        }
